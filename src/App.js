...
function App() {
    ...
    return (
      <div key={rowIndx} className="row">
        {row.map((column, colIndx) =>
          column == 0 ? (
            <div key={colIndx} className="column non-active">
              {}
            </div>
          ) : column == 1 ? (
            <div key={colIndx} className="column active">
              {}
            </div>
          ) : (
            <div key={colIndx} className="column apple">
              {}
            </div>
          )
        )}
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      let b = new Array(BOARD_SIZE)
        .fill(0)
        .map((cell) => new Array(BOARD_SIZE).fill(0));
      const snakePos = [...snake];

      let addLength = false;
      // Next Snake Move
      snakePos.push(
        nextMoveSnake(snakePos[snakePos.length - 1], moveDirection)
      );
  ...
  }, [moveDirection, board, gameOver, start]);
    ...
    <div className="Board">
      {!start && (
        <div className="play-btn-c">
          <button className="play-btn" onClick={() => setStart(true)}>
            PLAY
          </button>
        </div>
      )}
